 ...
 */
function autoAccountEmailsFromCMBC() {
    Logger.log('🕙 Start to Record Accounting Info...');
    ...
    const threads = GmailApp.search(unread_cmbc_email);

    if (threads.length === 0) {
        Logger.log("没有找到 CMBC 的未读邮件。");
        return;
    }
                ...
                switch (transaction.label) {
                    ...
                    case 'Finished':
                        ...
                        if (createAccountingRecord(payload)) {
                            ...
                            message.markRead();
                            Logger.log('✅ Transaction has been processed successfully.' + message.getPlainBody().toString());
                        } else {
                            message.markUnread();
                            labelErrors.addToThread(thread);
